...
[B<--ondemand>] [B<--delaymount>] [B<--reverse>] [B<--standard>] [B<--paranoia>]
[B<--require-macs>] [B<--nocache>] [B<--mount>] [B<--config=path>] 
[B<--password=pass>]
[B<-o FUSE_OPTION>]
I<rootdir> I<mountPoint> 
[B<--> [I<Fuse Mount Options>]]

B<encfs> B<--unmount> [B<I<options>>] 
I<mountPoint> 
[B<--> [I<Fuse Mount Options>]]
...
Specifies that B<EncFS> I<must> mount the given filesystem, meaning that the filesystem
must already exist.  B<EncFS> will not create the filesystem if it is non-existant 
with this option enabled. 

=item B<--unmount>

Unmount the filesystem as specified by I<mountPoint>.  Note that this option cannot be 
used with B<--mount> and must only specify I<mountPoint>. 

=item B<--config=path>

Specifies which config file to use, overriding environment variable ENCFS6_CONFIG.  Refer 
to the warnings specified by I<ENCFS6_CONFIG> below. 

=item B<--password=passwd>

Allows supplying of the password from the command line. If passwd contains spaces, 
it must be enclosed in quotation marks.  B<Warning>: This method of supplying a password 
may be insecure! (e.g., command prompt history is logged)

For example, this option can be combined with --paranoia or --standard to quickly 
create an encrypted filesystem without prompting: 

    encfs --standard --password=abc /home/me /tmp/crypt-view
...
or by running I<encfs> with the B<--unmount> option: 
